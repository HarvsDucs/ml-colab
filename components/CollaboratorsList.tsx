...
const CollaboratorsList: React.FC<CollaboratorsListProps> = ({ collaborators, className = '' }) => {
  return (
    <div className={`space-y-4 ${className}`}>
      <h3 className="text-lg font-medium text-gray-900">Current Collaborators</h3>
      <div className="space-y-3">
        {collaborators.map((collaborator) => (
          <div key={collaborator.id} className="flex items-center p-3 bg-gray-50 rounded-lg">
            ...
            {collaborator.githubUsername && (
              <a
                href={`https://github.com/${collaborator.githubUsername}`}
                target="_blank"
